<div id="deliveryReports">
    ...
    if (isset($fields) && count($fields) > 0){
        ...
        foreach ($fields as $field) {
            ...
            $values[] = [$i,$field->orderType,$field->foodcount,$field->totalFee];
            $i++;
        }
        $titles = ['ردیف','نوع تحویل','تعداد فروش','میزان فروش'];
        $tableFields = ['','z_order_types.name','foodcount','totalFee'];
        echo zTable($titles,$values,'titleli','titleul','rowli','rowul',$tableFields,$orderType,$orderField,url().'/admin/filterDeliveryReports','deliveryReports');
    }else{
        $titles = ['ردیف','نوع تحویل','تعداد فروش','میزان فروش'];
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="nav active"><a id="first" class="jtab" href="#firstDiv" data-toggle="tab" aria-expanded="true">نمودار تحویل / میزان فروش</a></li>
        <li class="nav"><a id="second" class="jtab" href="#secondDiv" data-toggle="tab">نمودار نحویل / تعداد فروش</a></li>
    </ul>
    <div class="jreportchartdiv" style="text-align: center;">
        <div id="firstDiv">
            <?php if (isset($title)&&isset($sales)){?>
                <?php echo highCharts('تحویل / میزان فروش',$title,'میزان فروش','del-sale','pie',$sales,true);?>
                <div id="del-sale" class="jreportchart jtransition active"></div>
            <?php } ?>
        </div>
        <div id="secondDiv">
            <?php if (isset($title)&&isset($solds)){?>
                <?php echo highCharts('تحویل / تعداد فروش',$title,'تعداد فروش','del-sold','pie',$solds,true);?>

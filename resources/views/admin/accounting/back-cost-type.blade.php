...
if (isset($fields)){
  ...
  foreach($fields as $field){
    ...
    <ul style="display: flex;justify-content: space-between;align-items: center;max-width: 600px; color:#FAFAFA;margin: 0 auto;text-align: center; color:#2f2f2f;">
      ...
      <li style="display: inline-block;padding:10px 8px;width:200px;"><?php echo $field->desc;?></li>
      <li style="padding:12px 8px;width:200px;display: inline-block;"><a href="javascript:void(0);" onclick="backcosttype('{{$field->id}}}')">بازگشت</a>
    </ul>
    <?php
    $i++;
  }
...
<script>
    function backcosttype(id){
    $('#loading').show();
    ajaxSendDataWithOutBack({
      itemId:id
    },'<?php echo url()?>/admin/back-costtype');
    setTimeout("responder('back-cost-type')",1500);

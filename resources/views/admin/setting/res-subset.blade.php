    ...
    if (isset($fields)){
        ...
        foreach($fields as $field){
            ...
            <tr >
                ...
                <td >
                    <a href="#" title="ویرایش" onclick="editressubset('<?php echo $field->id?>')"><i class="fa fa-pencil"></i></a>

                    <a href="javascript:void(0);" class="jtrashicon" title="حذف" onclick="deleteresssubset('<?php echo $field->id?>')"><i class="fa fa-trash"></i></a>
                </td>
            </tr>
            <?php
            $i++;
...
<script>
    $(function(){
        $('#add').on('click',function(){
            ajaxRequest('<?php echo url()?>/admin/new-res-subset','ReportDialog');
            setDialog('ReportDialog',600,400);
        });
    });
    function  deleteresssubset(id){
        $('#loading').show();
        ajaxSendDataWithOutBack({
            itemId:id
        },'<?php echo url()?>/admin/delete-res-subset');
        setTimeout("responder('subsets')",1500);
        setTimeout("$('#loading').hide()",1600);
    }
    function editressubset(id){
        ajaxRequest('<?php echo url()?>/admin/edit-res-subset/'+id,'ReportDialog');
        setDialog('ReportDialog',600,400);
    }
